...
export default function LearningPage() {
  ...
  useEffect(() => {
    ...
    // Verify enrollment
    if (!parsedUser.enrolledCourses?.includes(parseInt(params.courseId))) {
  ...
  const handleLessonComplete = () => {
    ...
    // If all lessons completed, update user's completed courses and show certificate
    if (newCompleted.length === course.curriculum.length) {
      const updatedUser = {
        ...user,
        completedCourses: [...(user.completedCourses || []), parseInt(params.courseId)],
        certificates: [...(user.certificates || []), parseInt(params.courseId)]
      };
      localStorage.setItem('user', JSON.stringify(updatedUser));
      router.push(`/dashboard/student/courses/${params.courseId}/certificate`);
    } else {
      // Move to next lesson
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3 bg-white rounded-lg shadow-md p-6">
            ...
            <div className="bg-gray-100 rounded-lg p-6 mb-6">
              ...
              >
                {activeLesson === course.curriculum.length - 1 ? 'Complete Course' : 'Complete & Continue'}
              </Button>
            </div>
          </div>
